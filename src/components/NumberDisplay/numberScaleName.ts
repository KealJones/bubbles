// Source: https://codereview.stackexchange.com/questions/244474/large-number-scales-names-generator

export function convertExponentToPower(num: number) {
  return Math.max(Math.floor((num - 3) / 3), 0);
}
/************************************************************************
 ...
 *                The largest scale name is therefor the number with
 ...
 *************************************************************************/
export function numberScaleName(Power = 0) {
  ...
  const Hund = Math.floor(Power / 100), // Hundred Digit
    ...
    UnitName = [
      ...
      "octo",
      "nove",
    ][Unit]; // Get Unit Name from Array
  let TenName = TensList[Ten][0], // Get Tens Name from Array
  ...
  // @ts-expect-error the boolean addition logic is correct
